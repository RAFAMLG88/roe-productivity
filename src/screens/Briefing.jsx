...
const metaCartao = (tarefa) => {
  ...
  const primeira = ks[0] ? metaTag(ks[0]) : null
...
export default function Briefing({ onNavigate }) {
  ...
  // capacidade de hoje encolhe com o passar do dia; refresca a cada minuto
  const [, tickCap] = useState(0)
  useEffect(() => { const t = setInterval(() => tickCap((x) => x + 1), 60000); return () => clearInterval(t) }, [])
